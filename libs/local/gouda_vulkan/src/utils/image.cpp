...
Expect<Image, std::string> Image::Load(std::string_view filename, int desired_channels)
{
    ...
    stbi_uc *data = stbi_load(filename.data(), &size.width, &size.height, &channels, desired_channels);
    if (!data) {
        return std::unexpected("Failed to load image");
    }

    // Copy image data to a vector
    std::vector<stbi_uc> image_data(data, data + size.width * size.height * channels);

    // Free the original data loaded by stbi_load
    stbi_image_free(data);

    // Return the Image object with the loaded data
    return Image(std::move(image_data), {size.width, size.height}, channels);
}
