/**
 * @file renderers/vulkan/vk_compute_pipeline.cpp
    ...
    : m_renderer{renderer}, p_device{renderer.GetDevice()}
{
    ...
    std::vector<VkDescriptorSetLayout> layouts(storage_buffers.size(), p_descriptor_set_layout);
...
void ComputePipeline::BindDescriptors(VkCommandBuffer command_buffer, u32 image_index) const
{
...
u32 ComputePipeline::CalculateWorkGroupCount(u32 particle_count) const
{
    const u32 local_size_x{256};                               // Matches shader's local_size_x
    return (particle_count + local_size_x - 1) / local_size_x; // Ceiling division
}

void ComputePipeline::Dispatch(VkCommandBuffer command_buffer, u32 group_count_x, u32 group_count_y,
                               u32 group_count_z) const
{
    vkCmdDispatch(command_buffer, group_count_x, 1, 1); // Simplify for 1D dispatch
}
